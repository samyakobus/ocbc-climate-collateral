...
def portfolio_sql(rows: Sequence[Dict[str, object]]) -> str:
    out: List[str] = []
    ...
    out.append("  dist_to_coast_km, adaptation_project_id")
    out.append(") VALUES")
    values = []
    for r in rows:
        ...
        values.append("  (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)" % (
            sql_literal(r["collateral_id"]), sql_literal(r["address_line"]),
...
def write_seed_sql(rows: Sequence[Dict[str, object]], repo: str) -> str:
    ...
    with open(path, "w", encoding="utf-8", newline="") as handle:
        handle.write(portfolio_sql(rows))
    return path
